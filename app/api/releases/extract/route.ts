...
import {
  getMessagesWithoutReleases,
  insertRelease,
  deleteReleasesForReextraction,
  type SlackMessage,
  type ThreadReply,
} from '@/lib/db/client'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // If reextract, clear old releases first
    if (reextract) {
      const deleted = await deleteReleasesForReextraction(promptVersion)
      console.log(`Deleted ${deleted} old releases for re-extraction`)
    }

    // Get messages that need extraction
    const messages = await getMessagesWithoutReleases(promptVersion)
